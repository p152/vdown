...
Вся навигация — через **inline-кнопки**: лимит, Premium, справка, поддержка, отмена загрузки.
...
**Важно:** группа в whitelist даёт доступ к боту в чате, но **не снимает** дневной лимит загрузок — лимиты считаются на каждого пользователя отдельно. Безлимит только у Premium, админов и пользователей в whitelist (не групп).
...
Настройка — только через web-админку (или файл `cookies/cookies.txt` на сервере).
